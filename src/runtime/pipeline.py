...
def ensure_fund_rank_representative(sql: str, question: str = "") -> tuple[str, bool]:
    ...
    if m_grp:
        ...
        #    (`COALESCE(…, itm_no)` 없는 리드)는 역외 110행의 mtco NULL 을 한 그룹으로 뭉쳐 랭킹을 흔들었고,
...
def ensure_fund_evidence_columns(sql: str) -> tuple[str, bool]:
    ...
    **재료 자체가 없다** — ensure_risk_name_column(채권)의 펀드판. ① 위험등급 코드가 SQL 에 쓰였으면
    ...
    #    FND-016 리드 오인(어느 상품의 값인지 답변기가 지어낸다)이다.
...
def _declared_name_literals(ctx) -> frozenset:
    ...
    pd_nm LIKE 리터럴 중 질문 밖 108건이 전부 이 집합 안에 있다 — ESG 녹/사/지 4리드 · 후/전환/풋 4리드 · 신종·영구·물가·분리채권·콜·콜마·(정부보증))."""
    key = id(ctx)
...
# 이름 조회 필터 — **좌변이 itm_nm** 인 LIKE/GLOB 만 (원형 · TRIM(itm_nm) · 공백무시 REPLACE 리드).
...
def _list_answer(sql: str, rows: str, n: int) -> str | None:
    """순자산순 펀드 목록(ensure_fund_list_grouping 리드)의 답변을 기계 조립한다. 아니면 None. HCX 0회.
...
def _bond_list_answer(sql: str, rows: str, n: int, question: str) -> str | None:
    ...
    **종목명을 하나도 옮기지 않고**, 조회 결과에 없는 '6등급 최고 6.23%' 를 규칙 문구에서 끌어와 답했다. 주의 문구 규칙은 목록에
    ...
    # 정렬 축 — ORDER BY 첫 키(MAX/MIN 감싼 대표행 리드 포함)
    axis_txt = ""
    # 방향은 `\s*` 로 받는다 — `ORDER BY srfc_irt DESC`(감싸지 않은 리드)에서 앞의 `\s*` 가 공백을 먹어
...
def _bond_issuer_count(sql: str) -> int | None:
    """채권 단순 목록의 발행사 수 — COUNT(DISTINCT TRIM(pd_pbcm)). 리드이 맞지 않으면 None."""
    if "domestic_bonds" not in sql or re.search(r"\b(?:union|having|join)\b|\(\s*select\b", sql, re.I):
        return None
    m = _SIMPLE_FROM_WHERE.search(sql)
    if not m:
...
def _is_topn_or_entity_axis(sql: str, axis_item: str) -> bool:
    ...
    JOIN 없는 R2 리드)에 3열이 붙고 조립기가 "5개 범주 · 펀드 3,040개 · 복수 범주 1,632건" 조작 통계를 만들었다.
    """
    ...
    try:
        for c in others:
            ...
            if alone == 0:
                ...
                # 7R S′ — 리터럴 추출은 `=`·`LIKE`·**`IN`** 세 리드 전부. 어느 것도 못 뽑으면 사람말(_humanize_cond)만 쓰고
...
def ensure_mgmt_code_predicate(sql: str, question: str, mgmt: tuple | None) -> tuple[str, str | None]:
    ...
    두 갈래:
      ...
      ⓑ 이름 리터럴에 브랜드가 없으면(W1 리드) 등호가 아니라 **브랜드 어간 역조회 코드 전부의 `IN`** 으로 렌더한다.
...
def ensure_fund_type_axis(sql: str, question: str) -> tuple[str, bool]:
    ...
    if _BTYP_AXIS_COLS.search(where_body):
        ...
        if len(axis) == 1 and re.fullmatch(rf"\(?\s*(?:{re.escape(qual)})?zrin_btyp_nm\s*(?:=\s*'{re.escape(val)}'|LIKE\s*'%{re.escape(val)}')\s*\)?",
            ...
            return sql, False                                   # 이미 확정식(한정자 포함 · 등호/LIKE 리드이 같다) — 손대지 않는다
...
def ensure_fund_distinct_count(sql: str, question: str) -> tuple[str, bool]:
    ...
    ③ 질문이 '펀드 … 몇 개/개수' 리드 ④ 질문에 '클래스' 없음(클래스 수를 물으면 불개입).
...
def ensure_fund_series_boundary(sql: str, question: str) -> tuple[str, bool]:
    ...
    if not re.search(r"\bitm_nm\b", sql, re.I):
        return sql, False
    # 일반 규칙(2R Q6): 호수 표기는 'N호' 와 **'N(자산유형)' / 'N[자산유형]'** 두 리드이 공존한다(실측 546행/207 대표번호 —
...
def _synonym_keys(ctx) -> dict:
    ...
    if out is None:
        ...
        ctx._syn_keys_cache = out   # ETF 담당 09-02: ctx 속성 캐시 (id 재활용 stale 방지)
    return out
...
def _ground(
    ...
    if pairs is None:
        ...
        ctx._match_keys_cache = pairs   # ETF 담당 09-02: ctx 속성 캐시 (id 재활용 stale 방지)
...
def resolve_ordinal_order_by(sql: str) -> tuple[str, bool]:
    ...
    🔴 범위: domestic_bonds 단일 FROM 만 — gold 서수 9건 중 8건이 UNION(ETF 교차)이고 UNION 의 ORDER BY 는 서수가 정석이며
       ETF 가드 테스트가 그 리드을 전제한다(2026-09-06 과적합 점검). JOIN·UNION·서브쿼리·SELECT * 불개입. 서수가 항 수를 넘으면 원문.
    SQLite 의미는 동일하다(서수 = 그 위치의 결과 열) — 되돌린 SQL 은 같은 행을 낸다(gold ETF-D-036 리드 대조)."""
    if "domestic_bonds" not in sql or re.search(r"\b(?:join|union)\b|\(\s*select\b", sql, re.I):
        return sql, False
...
def answer_question(
    ...
    *,
    ...
    def _regen(problem: str):
        ...
        #    같은 문장에 같은 가드·같은 검사를 두 번 태워도 결과는 같다(#87 리드). 판정을 남겨 다음 실측에서 세게 한다.
        if _norm_sql_text(sql2) == _norm_sql_text(sql):
            step("[Plan] 재생성 무효 — 정규화 후 직전 SQL 과 같은 문장이다(#87 리드). 같은 검사를 되풀이하지 않고 종료한다")
            return None
        if violations and viol2 and _violation_sig(viol2) == _violation_sig(violations):
            step("[Plan] 재생성 무효 — 직전과 같은 값 위반을 되풀이했다(#87 리드): "
    ...
    # ETF 답변 가드(백분율·거절 전사·집계 오거절) 뒤에 붙인다 — 전사 교체가 고지를 지우지 않게 (etf-dev 병합 검토 2026-09-06: "내 가드 → 리드 고지")
